...
Для каждого таска или баг фикса создается **feature** ветка от **master** или **dev** ветки, в зависимости от того какую кодовую базу необходимо расширить/пофиксить.

В название ветки должно входить краткое (не более трёх слов), разделяемое дефисами, описание и номер задачи, отделённый через нижнее прдчёркивание.

```
$ git checkout -b [task-name]_[task#] dev
```

Например для Jira:
```
test-branch-name_TP1
```
Для WorkSection:
```
test-branch-name_0000001
```


**Feature** ветка должна сливаться с **master** или **dev** веткой через merge request.

**Feature** ветку можно назвать любым именем, кроме `bugfix-*`, `master`, `dev`.
...
При коммите, в сообщении, указывается заголовок задачи и ссылка на него.
Например:

```
Task name:
https://projects.requestum.com/project/0000001/0000001
```
